...
ggpubr::gghistogram(s1clustersLyrics, 
                    ...
                    fill = "spotifycluster", 
                    palette = "simpsons",
                    xlab = "Normalised sentiment using Afinn lexicon", 
                    bins = 50, 
                    xtickslab.rt = 30,
                    legend = "hide",
                    rug = T) 
...
```{r s1sentimentcluster}
library(factoextra)
library(NbClust)
factoextra::fviz_nbclust(NbClust(select(s1clustersLyrics, valence, energy, sentiment), distance = "euclidean", method = "kmeans")) + theme_minimal() + ggtitle("NbClust's optimal number of clusters")
