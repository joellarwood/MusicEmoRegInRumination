...
library(factoextra)
s3ClusterVars <- s3explore %>% select(valence, energy, id) %>% drop_na()
factoextra::fviz_nbclust(NbClust(select(s3ClusterVars, -id), distance = "euclidean", method = "kmeans")) + theme_minimal() + ggtitle("NbClust's optimal number of clusters")


s3clusters <- s3ClusterVars %>% 
  mutate(KmeansCluster = fct_recode(factor(kmeans(select(s3ClusterVars, -id), 2)$cluster)))

ggpubr::ggscatter(s3clusters, 
                  x = "valence", 
                  y = "energy", 
                  color = "KmeansCluster",
                  star.plot = TRUE,
