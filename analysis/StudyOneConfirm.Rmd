...
sd(s1$PostInduction)
```
The manipulation/induction of sadness was succesful *t*(381) = 14.01, *p* < .001, *d* = 0.72, with scores rising from baseline (*M* = `r mean(s1$Baseline)`, *SD* = `r sd(s1$Baseline)`) to Post induction (*M* = `r mean(s1$PostInduction)`, *SD* = `r sd(s1$PostInduction)`)
