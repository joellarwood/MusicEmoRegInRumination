...
sd(s1$PostInduction)
```
The manipulation/induction of sadness was succesful `r apa::t_apa(s1Manipulation)` with scores rising from baseline (*M* = `r mean(s1$Baseline)`, *SD* = `r sd(s1$Baseline)`) to Post induction (*M* = `r mean(s1$PostInduction)`, *SD* = `r sd(s1$PostInduction)`)
