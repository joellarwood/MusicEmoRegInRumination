...
apaTables::apa.cor.table(data = select(s2explore, 
                                       ...
                                       valence, 
                                       energy), 
                         filename = "docs/s2correlations.doc")
...
s2clusters <- full_join(s2explore, select(VAclust, id, KmeansCluster), by = "id")
...
SpreadContingencyS2 <- MechPropSummary %>% 
...
fisher.multcomp(xtabs(~Entrainment + KmeansCluster, data = s2clusters))$p.value
fisher.multcomp(xtabs(~Memory + KmeansCluster, data = s2clusters))$p.value
fisher.multcomp(xtabs(~Conditioning + KmeansCluster, data = s2clusters))$p.value
fisher.multcomp(xtabs(~Contagion + KmeansCluster, data = s2clusters))$p.value
fisher.multcomp(xtabs(~Appraisal + KmeansCluster, data = s2clusters))$p.value

ggpubr::ggbarplot(data = SpreadContingencyS2, 
                  x = "Mechanism", 
                  y = "proportion", 
                  color = "cluster", 
                  fill = "cluster", 
...
## Rumination
```{r}
ggpubr::ggviolin(drop_na(s2clusters), 
                  x = "KmeansCluster",
                  y = "rumination", 
                  fill ="KmeansCluster",
                  palette = "simpsons",
...
kruskal.test(reg.use_1 ~ KmeansCluster, data = s2clusters)
...
s2ClusterSadness <- lmerTest::lmer(Sadness~1+Timepoint*KmeansCluster + (1 |id), data = s2ClusterLong)
tab_model(s2ClusterSadness)
anova(s2ClusterSadness)
plot_model(s2ClusterSadness, 
           type = "int")

emmeans::contrast(emmeans::emmeans(s2ClusterSadness, ~Timepoint*KmeansCluster), "pairwise", by = "KmeansCluster")
```
